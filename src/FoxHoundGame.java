...
public class FoxHoundGame {
     ...
     */
   public  static char swapPlayers(char currentTurn) {
    ...
    private static void gameLoop(int dim, String[] players) throws IOException {
        ...
        while(!exit) {
            ...
            int choice = FoxHoundUI.mainMenuQuery(turn, STDIN_SCAN);  //returns -1 if invalid
                                                                      //returns 0 if valid
            ...
            switch(choice) {
                ...
                case FoxHoundUI.MENU_SAVE_GAME:
                    boolean saved = false;
                    while (!saved) {
                        Path saveFile = FoxHoundUI.fileQuery(STDIN_SCAN);
                        if (saveFile == null)
                            saved = false;
                        else
                            saved = FoxHoundIO.saveGame(players, turn, saveFile);
                    }
                    break;
                case FoxHoundUI.MENU_LOAD_GAME:
                    Path loadFile = FoxHoundUI.fileQuery(STDIN_SCAN);
                    if (loadFile == null) {
                        System.err.println("Load file is NULL");
                    }
                    else {
                        FoxHoundIO.loadGame(players, loadFile);
                    }
                    break;
                default:
